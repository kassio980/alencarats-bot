...
async function exibir(c, i, tipo) {
  ...
  if (!lista.length) {
    return i.reply({ content: '❌ Nenhum item cadastrado!', ephemeral: true });
  }

  const emb = new EmbedBuilder().setColor(c.config.cor_embed);
  if (tipo === 'produtos') {
    emb.setTitle('🛒 NOSSA LOJA');
  } else {
    emb.setTitle('🎫 ENTRADAS VIP');
  }

  emb.setDescription(
    lista.map(x => `• **${x.nome}** | R$${x.preco.toFixed(2)}\n${x.descricao || ''}`).join('\n\n')
  );
...
module.exports = {
  customId: 'loja_ver',
  async execute(c, i) {
    await exibir(c, i, 'produtos');
  },

  customId: 'loja_entradas',
  async execute(c, i) {
    await exibir(c, i, 'entradas');
  },
  ...
  async execute(c, i) {
    ...
    if (!car.length) {
      return i.reply({ content: '🛒 Carrinho vazio!', ephemeral: true });
    }
