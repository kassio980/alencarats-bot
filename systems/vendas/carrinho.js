...
module.exports={
  ...
  async execute(c,i){
    ...
    i.reply({content:`✅ **${item.nome}** adicionado!\nTotal atual: R$${car.reduce((s,x)=>s+x.preco,0).toFixed(2)}`,ephemeral:true});
  },
  customId:'limpar_carrinho',async execute(c,i){
    await c.db.delete(`carrinho_${i.user.id}`);
    i.reply({content:'🗑️ Carrinho limpo!',ephemeral:true});
  },
  customId:'finalizar_compra',async execute(c,i){
    const car = await c.db.get(`carrinho_${i.user.id}`)||[];
    const pix = await c.db.get('pix');
    if(!pix) return i.reply({content:'❌ PIX não configurado! Use `+venda config pix`',ephemeral:true});
    const total = car.reduce((s,x)=>s+x.preco,0);
    const idPedido = 'PED'+Date.now().toString().slice(-6);
    const canal = await i.guild.channels.create({name:`compra-${i.user.username}`,parent:c.config.categoria_compras,
    permissionOverwrites:[{id:i.guild.id,deny:['ViewChannel']},{id:i.user.id,allow:['ViewChannel','SendMessages']},{id:c.user.id,allow:['ViewChannel','ManageChannels','SendMessages']},...c.config.cargos_admin.map(r=>({id:r,allow:['ViewChannel','SendMessages']}))]});
    await canal.send({embeds:[new EmbedBuilder().setColor('#00ff00').setTitle('🛒 NOVO PEDIDO').addFields(
    ...
    await c.db.delete(`carrinho_${i.user.id}`);
    i.reply({content:`✅ Pedido criado! Acesse: <#${canal.id}>`,ephemeral:true});
  },
  ...
  async execute(c,i){
    ...
    try{const u = await c.users.fetch(pedidos[idx].cliente);await u.send(`✅ Seu pedido #${id} foi ${acao}!`)}catch(e){}
    i.reply(`✅ Pedido ${acao}!`);
